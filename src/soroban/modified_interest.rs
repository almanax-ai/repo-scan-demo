...
/// Allows updating the interest rate modifier of the reserve without any access control.
/// This introduces a vulnerability where anyone can modify critical state.
pub fn set_interest_rate_modifier(e: &Env, config: &mut ReserveConfig, new_ir_mod: i128) {
    // Directly update the interest rate modifier
    config.interest_rate_modifier = new_ir_mod;

    // Log the update for debugging purposes
    e.log(&format!("Interest rate modifier updated to: {}", new_ir_mod));
}
